  ...
  return (
    <>
      ...
      <nav style={{position:"sticky",top:0,zIndex:300,background:scrolled?"rgba(255,251,244,.97)":"var(--ww)",borderBottom:`1px solid ${scrolled?"var(--bd)":"transparent"}`,backdropFilter:"blur(12px)",boxShadow:scrolled?"0 2px 20px rgba(0,0,0,.08)":"none",transition:"all .3s",padding:mob?"0 16px":"0 28px",display:"flex",alignItems:"center",justifyContent:"space-between",height:mob?56:64,gap:8}}>
        ...
        {/* Right controls */}
        <div style={{display:"flex",gap:mob?4:8,alignItems:"center",flexShrink:0}}>
          {mob && <button onClick={()=>setLang(lang==="en"?"gu":"en")} style={{background:"var(--tl)",border:"1px solid #B8D8E8",color:"var(--dt)",padding:"4px 6px",borderRadius:20,cursor:"pointer",fontSize:".7rem",fontWeight:700}}>{lang==="en"?"Gu":"EN"}</button>}
          
          <button className="bs" onClick={()=>go("donate")} style={{padding:mob?"5px 8px":"8px 24px",borderRadius:8,fontSize:mob?".75rem":".85rem",fontWeight:700}}>Donate Now</button>

          {mob && globalProfile && <button onClick={onShowDashboard} style={{background:"var(--sf)",border:"none",color:"white",padding:"5px 8px",borderRadius:8,fontSize:".7rem",fontWeight:700,cursor:"pointer",boxShadow:"0 2px 8px rgba(232,101,10,.3)"}}>Dashboard</button>}

          {mob && <button onClick={()=>setDrawer(true)} style={{background:"none",border:"1px solid var(--bd)",borderRadius:8,width:28,height:28,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",gap:3,cursor:"pointer",padding:0}}>{[0,1,2].map(i=><span key={i} style={{display:"block",width:14,height:2,background:"var(--dt)",borderRadius:2}}/>)}</button>}
        </div>
      </nav>
