...
const DC = {
  ...
  headerAction: {
    mode: "dashboard", // "donate" | "dashboard" | "custom" | "none"
    label: "Dashboard",
    labelGu: "ડેશબોર્ડ",
    linkTarget: "events"
  },
...
function Events({ C, lang, globalAuthToken, globalProfile, onPublicLogin, forceStandalone }) {
  ...
  const handleFileUpload = async (e, fKey) => {
    ...
    try {
      ...
      setFormData(prev => {
        ...
        return { ...prev, [fKey]: combined.join(",") };
      });
    } catch (err) {
      console.error(err);
      alert("Failed to upload file(s): " + err.message);
    } finally {
  ...
  if (forceStandalone) {
    return (
      <div style={{background:"white",borderRadius:16,padding:mob?"16px":"28px",boxShadow:"0 8px 32px rgba(0,0,0,0.08)",border:"1px solid var(--bd)"}}>
        {selectedEvent && selectedEvent.type === 'register' ? (
          <div>
            <h3 style={{fontFamily:"'Playfair Display',serif",fontSize:"1.4rem",color:"var(--dt)",marginBottom:4,fontWeight:700}}>
              {authStep === 'complete_profile' ? 'Complete Profile' : authStep === 0 ? 'User Login' : selectedEvent.event.title}
            </h3>
            <p style={{fontSize:".85rem",color:"var(--mu)",marginBottom:20}}>
              {authStep === 1 ? 'Please fill in the details below to complete your registration' : 'Sign in or create a profile to continue'}
            </p>
            {selectedEvent.event.regClosed ? (
              <div style={{textAlign:"center",padding:"36px 20px",background:"linear-gradient(135deg,#FFF5F5,#FFF0F0)",borderRadius:12,border:"1px solid #F5B8B8"}}>
                <div style={{fontSize:"3rem",marginBottom:12}}>🚫</div>
                <h4 style={{color:"#C0392B",fontWeight:700,marginBottom:8,fontSize:"1.1rem"}}>Registration Closed</h4>
                <p style={{fontSize:".9rem",color:"#666",lineHeight:1.6}}>
                  {selectedEvent.event.regClosedMsg || "New registration entries have been stopped. Please contact the organizer for more information."}
                </p>
              </div>
            ) : done ? (
              ...
              <form onSubmit={handleSendOtp} style={{display:"flex",flexDirection:"column",gap:16}}>
                ...
                <div>
                  ...
                  <div style={{display:"flex",gap:8}}>
                    <select value={countryCode} onChange={e=>setCountryCode(e.target.value)} style={{padding:"10px",borderRadius:8,border:"1px solid #CCC",fontSize:".85rem",fontFamily:"inherit",background:"white"}}>
                      <option value="+91">+91 (IN)</option>
                      <option value="+1">+1 (US)</option>
                      <option value="+44">+44 (UK)</option>
                      <option value="+971">+971 (UAE)</option>
                    </select>
                    <input type="tel" required value={mobile} onChange={e=>setMobile(e.target.value)} placeholder="10-digit number" style={{flex:1,padding:"10px",borderRadius:8,border:"1px solid #CCC",fontSize:".85rem",fontFamily:"inherit"}}/>
                  </div>
                </div>
                {authError && <div style={{background:"#FDECEA",color:"#C0392B",padding:"10px",borderRadius:8,fontSize:".8rem",fontWeight:600}}>{authError}</div>}
                {!otpSent ? (
                  <button type="submit" className="bs" style={{padding:"12px",borderRadius:8,fontWeight:700,opacity:submitting?0.7:1}} disabled={submitting}>
                    {submitting ? "Sending OTP..." : "Send OTP"}
                  </button>
                ) : (
                  <div style={{display:"flex",flexDirection:"column",gap:12}}>
                    <input type="text" value={otp} onChange={e=>setOtp(e.target.value)} placeholder="Enter 6-digit OTP" style={{padding:"10px",borderRadius:8,border:"1px solid #CCC",fontSize:".85rem",fontFamily:"inherit",textAlign:"center"}}/>
                    <button type="button" onClick={handleVerifyOtp} className="bs" style={{padding:"12px",borderRadius:8,fontWeight:700,opacity:submitting?0.7:1}} disabled={submitting}>
                  ...
                  return (
                    <>
                      ...
                      {formObj.instructions && (
                        <div className="rich-instructions" style={{gridColumn:"1 / -1", marginBottom: 14, background: "#FFFBF4", border: "1px solid #FFE8C5", padding: "12px 16px", borderRadius: 8, fontSize: ".85rem", color: "var(--tx)", lineHeight: 1.5}} dangerouslySetInnerHTML={{__html: cleanFormInstructions(formObj.instructions)}} />
                      )}
                    </>
                  );
                })()} 
                  ...
                  if (logicRules.length > 0) {
                      shouldShow = (f.logicCondition === "AND" ? logicRules.every : logicRules.some).call(logicRules, rule => {
                          ...
                          if (parentField && parentIdx !== -1) {
                              const firstIdx = allF.findIndex(ff => ((ff.dataKey || ff.label)?.trim()) === parentKey);
                          ...
                          const pValStr = String(parentVal).trim().toLowerCase();
                          const dValStr = String(rule.dependsValue).trim().toLowerCase();
                          return pValStr === dValStr;
                      });
                  }
                  if (!shouldShow) return null;
                  
                  let fKey = (f.dataKey || f.label)?.trim() || `Field_${idx + 1}`;
                  const allF1 = getForm(selectedEvent.event.formId).fields;
                  const firstMatchingIdx1 = allF1.findIndex(ff => ((ff.dataKey || ff.label)?.trim()) === fKey);
                  if (firstMatchingIdx1 !== -1 && firstMatchingIdx1 !== idx) {
                      fKey = `${fKey}_${idx + 1}`;
                  }
                          ...
                          return (
                            <div style={{display:"flex",flexDirection:"column",gap:8}}>
                              {uploadedUrls.map((url, uIdx) => (
                                <div key={uIdx} style={{display:"flex",alignItems:"center",gap:10,background:"white",padding:"8px 12px",borderRadius:8,border:"1px solid var(--bd)"}}>
                                  {f.type === 'image' || url.match(/\.(jpg|jpeg|png|webp|gif)/i) ? (
                                    <img src={url} alt="preview" style={{width:40,height:40,objectFit:"cover",borderRadius:6,border:"1px solid var(--bd)"}}/>
                                  ) : (
                                    <div style={{width:40,height:40,background:"#E2E8F0",borderRadius:6,display:"flex",alignItems:"center",justifyContent:"center",fontSize:".65rem",fontWeight:800,color:"#334155"}}>DOC</div>
                                  )}
                                  <div style={{flex:1,minWidth:0}}>
                                    <div style={{fontSize:".85rem",fontWeight:600,color:"var(--dt)",whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis"}}>
                                      Document {uIdx + 1}
                                    </div>
                                    <button type="button" onClick={()=>setPreviewFile({url, type:f.type})} style={{background:"none",border:"none",fontSize:".75rem",color:"#2563EB",textDecoration:"underline",cursor:"pointer",padding:0}}>
                                      👁️ View File
                                    </button>
                                  </div>
                                  <button type="button" title="Delete document" onClick={()=>{
                                    const remaining = uploadedUrls.filter((_, idx) => idx !== uIdx);
                                    setFormData({...formData, [fKey]: remaining.join(",")});
                                  }} style={{background:"#FEF2F2",border:"1px solid #FCA5A5",color:"#DC2626",borderRadius:6,cursor:"pointer",fontSize:".75rem",fontWeight:700,padding:"4px 10px",display:"flex",alignItems:"center",gap:4}}>
                                    🗑️ Delete
                                  </button>
                                </div>
                              ))}
                              ...
                              {uploadingFields[fKey] ? (
                                <div style={{fontSize:".85rem",color:"var(--mu)",fontStyle:"italic",display:"flex",alignItems:"center",gap:8,padding:"6px 0"}}>
          ...
          <div style={{textAlign:"center",padding:40,color:"var(--mu)"}}>Loading Event Registration Form...</div>
        )}
      </div>
    );
  }
                ...
                {selectedEvent.event.regClosed ? (
                  ...
                  <form onSubmit={submitForm} style={{display:"grid",gridTemplateColumns: mob ? "1fr" : "1fr 1fr",gap:12, rowGap:16}}>
                    ...
                    {getForm(selectedEvent.event.formId).fields.map((f, idx) => {
                      ...
                      let fKey = (f.dataKey || f.label)?.trim() || `Field_${idx + 1}`;
                      const allF2 = getForm(selectedEvent.event.formId).fields;
                      const firstMatchingIdx2 = allF2.findIndex(ff => ((ff.dataKey || ff.label)?.trim()) === fKey);
                      if (firstMatchingIdx2 !== -1 && firstMatchingIdx2 !== idx) {
                          fKey = `${fKey}_${idx + 1}`;
                      }
                      const spanFull = f.type === 'address' || f.type === 'file' || f.type === 'image' || f.type === 'fullname';
                      return (
...
function ContentEditor({ C, setC, setPage, auth, hasAccess, master }) {
  ...
  return (
    ...
    <div>
      ...
      {/* ── SEO & META TAGS MANAGER (PROMINENT ACCORDION) ─────────────────── */}
      <Sec id="seo" icon="🔍" label="SEO & Search Engine Optimization (Google & WhatsApp)">
        <G2>
          <F label="Browser Page Title (<title>)" path="seoTitle" hint="Shown in browser tab and search engine results"/>
          <F label="Author / Organization Name" path="seoAuthor" hint="e.g. Mumbai Meghwal Panchayat (MMP)"/>
        </G2>
...
// ── ADMIN SEO TAB ─────────────────────────────────────────────────────────────
function AdminSeoTab({ C, setC, auth, mob }) {
  const defaultTitle = C?.trust?.name ? `${C.trust.name} | Community Portal` : "Community Portal";
  const defaultDesc = C?.trust?.name ? `Official Community Portal for ${C.trust.name} - Event Registrations, Education Awards, and Donations.` : "Official Community Portal";
  const defaultAuthor = C?.trust?.name || "";
  const defaultKeywords = C?.trust?.name ? `${C.trust.name}, Community Portal` : "";
  const defaultCanonical = window.location.origin;

  const [seo, setSeo] = useState({
    seoTitle: C?.seoTitle || defaultTitle,
    seoAuthor: C?.seoAuthor || defaultAuthor,
    seoDescription: C?.seoDescription || defaultDesc,
    seoKeywords: C?.seoKeywords || defaultKeywords,
    seoCanonical: C?.seoCanonical || defaultCanonical,
    seoOgTitle: C?.seoOgTitle || defaultTitle,
    seoOgDesc: C?.seoOgDesc || defaultDesc
  });
  ...
  const handleSave = async () => {
    ...
    setC(newC);
    if (!auth?.idToken) {
      setMsg({ type: "warn", text: "Changes saved locally. Login to publish online." });
      return;
    }
    setSaving(true);
    try {
      await fbSave(newC, auth.idToken);
      setMsg({ type: "saved", text: "SEO & Search Metadata saved to database!" });
    } catch (e) {
  ...
  return (
    <div style={{background:"white", borderRadius:16, padding:mob?16:32, boxShadow:"0 4px 20px rgba(0,0,0,0.05)", border:"1px solid var(--bd)"}}>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:24,flexWrap:"wrap",gap:16,borderBottom:"1px solid var(--bd)",paddingBottom:16}}>
        <div>
          <h2 style={{fontFamily:"'Playfair Display',serif",color:"var(--dt)",margin:0,fontSize:"1.5rem",display:"flex",alignItems:"center",gap:8}}>
            <span>🔍</span> SEO & Search Engine Settings
          </h2>
          <p style={{color:"var(--mu)",fontSize:".85rem",marginTop:4}}>Manage Google search rankings, keywords, page title, and WhatsApp link previews.</p>
        </div>
        <button onClick={handleSave} disabled={saving} style={{padding:"12px 24px",borderRadius:10,background:"var(--sf)",color:"white",border:"none",fontWeight:700,fontSize:".95rem",cursor:saving?"wait":"pointer",boxShadow:"0 4px 14px rgba(232,101,10,0.3)"}}>
          {saving ? "Saving..." : "💾 Save SEO Settings"}
        </button>
      </div>

      {msg && (
        <div style={{padding:"12px 18px",borderRadius:8,marginBottom:20,fontWeight:600,fontSize:".85rem",background:msg.type==="error"?"#FEF0EF":msg.type==="warn"?"#FFF8E1":"#EDFAF1",color:msg.type==="error"?"#C0392B":msg.type==="warn"?"#C8860A":"#1A7A3E",border:`1px solid ${msg.type==="error"?"#F5B8B8":msg.type==="warn"?"#FFE082":"#B8E8CC"}`}}>
          {msg.text}
        </div>
      )}
      ...
      <div style={{display:"flex",flexDirection:"column",gap:20}}>
        <div>
          <label style={{display:"block",fontSize:".8rem",fontWeight:700,color:"var(--dt)",marginBottom:6,textTransform:"uppercase",letterSpacing:.5}}>Browser Page Title (&lt;title&gt;)</label>
          <input type="text" value={seo.seoTitle} onChange={e=>setSeo({...seo, seoTitle: e.target.value})} style={{width:"100%",padding:"11px 14px",borderRadius:8,border:"1px solid var(--bd)",fontSize:".9rem",fontFamily:"inherit"}} placeholder="Community Portal"/>
          <div style={{fontSize:".75rem",color:"var(--mu)",marginTop:4}}>Appears on browser tabs & as the main blue link on Google.</div>
        </div>

        <div>
          <label style={{display:"block",fontSize:".8rem",fontWeight:700,color:"var(--dt)",marginBottom:6,textTransform:"uppercase",letterSpacing:.5}}>Author / Organization Name</label>
          <input type="text" value={seo.seoAuthor} onChange={e=>setSeo({...seo, seoAuthor: e.target.value})} style={{width:"100%",padding:"11px 14px",borderRadius:8,border:"1px solid var(--bd)",fontSize:".9rem",fontFamily:"inherit"}} placeholder="Organization Name"/>
        </div>

        <div>
          <label style={{display:"block",fontSize:".8rem",fontWeight:700,color:"var(--dt)",marginBottom:6,textTransform:"uppercase",letterSpacing:.5}}>Meta Description</label>
          <textarea value={seo.seoDescription} onChange={e=>setSeo({...seo, seoDescription: e.target.value})} style={{width:"100%",padding:"11px 14px",borderRadius:8,border:"1px solid var(--bd)",fontSize:".9rem",fontFamily:"inherit",minHeight:80,resize:"vertical"}} placeholder="Summarizes your site for Google search results..."/>
          <div style={{fontSize:".75rem",color:"var(--mu)",marginTop:4}}>Appears under the title on Google search engine results pages.</div>
        </div>

        <div>
          <label style={{display:"block",fontSize:".8rem",fontWeight:700,color:"var(--dt)",marginBottom:6,textTransform:"uppercase",letterSpacing:.5}}>Meta Keywords (Comma Separated)</label>
          <textarea value={seo.seoKeywords} onChange={e=>setSeo({...seo, seoKeywords: e.target.value})} style={{width:"100%",padding:"11px 14px",borderRadius:8,border:"1px solid var(--bd)",fontSize:".9rem",fontFamily:"inherit",minHeight:70,resize:"vertical"}} placeholder="Organization, Trust, NGO, Community"/>
          <div style={{fontSize:".75rem",color:"var(--mu)",marginTop:4}}>Search terms users type on Google to find your website.</div>
        </div>

        <div>
          <label style={{display:"block",fontSize:".8rem",fontWeight:700,color:"var(--dt)",marginBottom:6,textTransform:"uppercase",letterSpacing:.5}}>Canonical Site URL</label>
          <input type="text" value={seo.seoCanonical} onChange={e=>setSeo({...seo, seoCanonical: e.target.value})} style={{width:"100%",padding:"11px 14px",borderRadius:8,border:"1px solid var(--bd)",fontSize:".9rem",fontFamily:"inherit"}} placeholder="https://www.example.com/"/>
        </div>

        <div style={{background:"#F8FAFC",border:"1px solid #E2E8F0",borderRadius:12,padding:20,marginTop:10}}>
          <h4 style={{margin:0,marginBottom:12,color:"var(--dt)",fontSize:".95rem",fontWeight:700}}>💬 WhatsApp & Social Media Preview Cards</h4>
          
          <div style={{marginBottom:14}}>
            <label style={{display:"block",fontSize:".78rem",fontWeight:700,color:"var(--dt)",marginBottom:6}}>Social Share Title (OG Title)</label>
            <input type="text" value={seo.seoOgTitle} onChange={e=>setSeo({...seo, seoOgTitle: e.target.value})} style={{width:"100%",padding:"10px 12px",borderRadius:8,border:"1px solid var(--bd)",fontSize:".85rem",fontFamily:"inherit",background:"white"}} placeholder="Organization Name"/>
          </div>

          <div>
            <label style={{display:"block",fontSize:".78rem",fontWeight:700,color:"var(--dt)",marginBottom:6}}>Social Share Description (OG Description)</label>
            <textarea value={seo.seoOgDesc} onChange={e=>setSeo({...seo, seoOgDesc: e.target.value})} style={{width:"100%",padding:"10px 12px",borderRadius:8,border:"1px solid var(--bd)",fontSize:".85rem",fontFamily:"inherit",minHeight:60,background:"white",resize:"vertical"}} placeholder="Official Community Portal..."/>
          </div>
        </div>

        <button onClick={handleSave} disabled={saving} style={{width:"100%",padding:"14px",borderRadius:10,background:"var(--dt)",color:"white",border:"none",fontWeight:700,fontSize:"1rem",cursor:saving?"wait":"pointer",marginTop:10,boxShadow:"0 4px 14px rgba(13,75,94,0.2)"}}>
          {saving ? "Saving Changes..." : "💾 Save & Publish SEO Settings"}
        </button>
      </div>
    </div>
  );
...
function AdminForms({ C, setC, saveToFb, mob, auth }) {
  ...
  const normalizeForms = (rawList) => {
    if (!Array.isArray(rawList)) return [];
    return rawList.map((f, idx) => ({
      ...f,
      id: f.id || f.formId || ("form_" + idx),
      fields: Array.isArray(f?.fields) ? f.fields : []
    }));
  };

  const [forms, setForms] = useState(() => normalizeForms(C.forms));
  ...
  useEffect(() => {
    setForms(normalizeForms(C.forms));
  ...
  const createForm = () => {
    ...
    setEditingFormIdx(nextForms.length - 1);
  };

  const updateCurrentForm = (updatedForm) => {
    setEditingForm(updatedForm);
    if (editingFormIdx !== null) {
      const nextForms = forms.map((f, i) => i === editingFormIdx ? updatedForm : f);
      upd(nextForms);
    }
  };

  const executeDeleteForm = (targetIdx) => {
    const nextForms = forms.filter((_, i) => i !== targetIdx);
    upd(nextForms);
    setConfirmDeleteIdx(null);
    if (editingFormIdx === targetIdx) {
      setEditingForm(null);
  ...
  const handleCreateStandardField = () => {
    ...
    if (editingForm) {
      const updated = {...editingForm, fields: [...(editingForm.fields || []), { label: nf.label, type: nf.type, options: nf.options, required: false }]};
      updateCurrentForm(updated);
    }
  ...
  return (
    <div style={{marginBottom: 30}}>
       ...
       <div style={{display:"flex",flexDirection:"column",gap:10}}>
          {forms.map((f, formIdx) => (
            <div key={f.id || formIdx} className="ac" style={{padding: "14px 18px", display:"flex", justifyContent:"space-between", alignItems:"center"}}>
              <div>
                <div style={{fontWeight:700,fontSize:".95rem",color:"var(--dt)"}}>{f.name}</div>
                <div style={{fontSize:".75rem",color:"var(--mu)",marginTop:2}}>{(f?.fields || []).length} fields</div>
              </div>
              <div style={{display:"flex",gap:8,alignItems:"center"}}>
                <button type="button" onClick={() => setPreviewForm(f)} style={{padding:"6px 14px",background:"#F0F4F8",border:"1px solid #CBD5E1",borderRadius:6,color:"#2B6CB0",cursor:"pointer",fontSize:".75rem",fontWeight:700}}>Preview</button>
                <button type="button" onClick={() => { setEditingForm(f); setEditingFormIdx(formIdx); }} style={{padding:"6px 14px",background:"var(--tl)",border:"1px solid var(--bd)",borderRadius:6,color:"var(--dt)",cursor:"pointer",fontSize:".75rem",fontWeight:700}}>Edit</button>
                
                {confirmDeleteIdx === formIdx ? (
                  <div style={{display:"flex", gap:4, alignItems:"center"}}>
                    <button type="button" onClick={() => executeDeleteForm(formIdx)} style={{padding:"6px 12px",background:"#C0392B",border:"none",borderRadius:6,color:"white",cursor:"pointer",fontSize:".75rem",fontWeight:700}}>Confirm</button>
                    <button type="button" onClick={() => setConfirmDeleteIdx(null)} style={{padding:"6px 8px",background:"#EEE",border:"none",borderRadius:6,color:"#333",cursor:"pointer",fontSize:".75rem"}}>Cancel</button>
                  </div>
                ) : (
                  <button type="button" onClick={() => setConfirmDeleteIdx(formIdx)} style={{padding:"6px 14px",background:"#FEF0EF",border:"1px solid #F5B8B8",borderRadius:6,color:"#C0392B",cursor:"pointer",fontSize:".75rem",fontWeight:700}}>Delete</button>
                )}
              </div>
            </div>
          ))}
       ...
       {editingForm && (
         <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.6)",zIndex:99999,display:"flex",alignItems:"center",justifyContent:"center",padding:16}}>
           <div className="ac" style={{background:"#FFFFFF",width:"100%",maxWidth:650,padding:24,borderRadius:14,maxHeight:"92vh",overflowY:"auto",position:"relative",boxShadow:"0 20px 50px rgba(0,0,0,0.3)"}}>
             <button onClick={() => { setEditingForm(null); setEditingFormIdx(null); }} style={{position:"absolute",top:16,right:16,background:"#F5F5F5",border:"none",fontSize:"1.2rem",cursor:"pointer",width:32,height:32,borderRadius:"50%",display:"flex",alignItems:"center",justifyContent:"center",color:"var(--mu)",zIndex:10}}>✕</button>
             
             <h3 style={{fontFamily:"'Playfair Display',serif",fontSize:"1.3rem",color:"var(--dt)",marginBottom:16,fontWeight:700,paddingRight:30}}>
               Edit Form Template
             </h3>

             <div style={{display:"flex", flexDirection:"column", gap:14}}>
               <div>
                 <label style={{display:"block",fontSize:".75rem",fontWeight:700,color:"var(--dt)",marginBottom:4}}>Form Name</label>
                 <input value={editingForm.name} onChange={e => updateCurrentForm({...editingForm, name: e.target.value})} style={{padding:"8px 12px",border:"1px solid var(--bd)",borderRadius:6,fontWeight:600,width:"100%",fontSize:".95rem"}} placeholder="Form Name"/>
               </div>

               <div style={{display:"flex", flexDirection:"column", gap:10, background:"#F9F9F9", padding:14, borderRadius:8, border:"1px solid var(--bd)"}}>
                 <div>
                   <label style={{display:"block",fontSize:".75rem",fontWeight:700,color:"var(--dt)",marginBottom:4}}>Banner/Header Image</label>
                   <div style={{display:"flex", gap:8, alignItems:"center"}}>
                     <input value={editingForm.bannerImage||""} onChange={e => updateCurrentForm({...editingForm, bannerImage: e.target.value})} style={{flex:1, padding:"8px",border:"1px solid var(--bd)",borderRadius:6,fontSize:".85rem"}} placeholder="Paste image URL or upload ->"/>
                     <label style={{padding:"8px 14px", background:"var(--dt)", color:"white", borderRadius:6, fontSize:".8rem", fontWeight:600, cursor:"pointer", whiteSpace:"nowrap"}}>
                       ...
                       <input type="file" accept="image/*" style={{display:"none"}} onChange={async (e) => {
                         ...
                         try {
                           const url = await fbUploadPublicFile(file, auth?.idToken);
                           updateCurrentForm({...editingForm, bannerImage: url});
                         } catch(err) {
                   ...
                   {editingForm.bannerImage && <img src={editingForm.bannerImage} alt="Banner Preview" style={{marginTop:8, width:"100%", maxHeight:100, objectFit:"cover", borderRadius:6, border:"1px solid var(--bd)"}}/>}
                 </div>
                 ...
                 <div>
                   ...
                   <div style={{background:"white", borderRadius:6, overflow:"hidden", border:"1px solid var(--bd)"}}>
                     ...
                     }} value={editingForm.instructions || ""} onChange={(content) => updateCurrentForm({...editingForm, instructions: content})} placeholder="Enter guidelines, bullet points, rules, or highlighted notes for users..."/>
                   </div>
                 </div>
               </div>
               ...
               {/* FIELDS BUILDER */}
               <div>
                 <label style={{display:"block",fontSize:".8rem",fontWeight:700,color:"var(--dt)",marginBottom:8}}>Form Fields ({(editingForm?.fields || []).length})</label>
                 <div style={{display:"flex",flexDirection:"column",gap:8,marginBottom:12}}>
                   {(editingForm?.fields || []).map((field, fieldIdx) => (
                     <div key={fieldIdx} style={{display:"flex",flexDirection:"column", transition:"transform 0.2s"}}
                          draggable
                          onDragStart={() => { dragItem.current = fieldIdx; }}
                          onDragEnter={() => { dragOverItem.current = fieldIdx; }}
                          onDragEnd={() => {
                     ...
                     >
                       <div style={{display:"flex",gap:8,alignItems:"center",background:"#F5F5F5",padding:"8px 12px",borderRadius:6}}>
                         {editingFieldId === `${editingForm.id}_${fieldIdx}` ? (
                            ...
                            <div style={{flex:1, display:"flex", alignItems:"center", gap: 8}}>
                              ...
                              <button onClick={() => setEditingFieldId(`${editingForm.id}_${fieldIdx}`)} title="Edit Field" style={{background:"none",border:"none",color:"var(--mu)",cursor:"pointer",fontSize:".9rem"}}>✏️</button>
                            </div>
                         )}
                         ...
                         <label style={{fontSize:".75rem",display:"flex",alignItems:"center",gap:4,marginLeft:10,fontWeight:600,color:"var(--dt)",cursor:"pointer"}}><input type="checkbox" checked={field.required} onChange={e => {
                           const newF = [...editingForm.fields]; newF[fieldIdx].required = e.target.checked; updateCurrentForm({...editingForm, fields:newF});
                         }}/> Req</label>
                         <button disabled={fieldIdx === 0} onClick={() => {
                           const newF = [...editingForm.fields];
                           const temp = newF[fieldIdx-1];
                           newF[fieldIdx-1] = newF[fieldIdx];
                           newF[fieldIdx] = temp;
                           updateCurrentForm({...editingForm, fields:newF});
                         }} style={{background:"none",border:"none",color:fieldIdx===0?"#ccc":"var(--dt)",cursor:fieldIdx===0?"default":"pointer",fontSize:"1.2rem",marginLeft:10,lineHeight:1}}>↑</button>
                         
                         <button disabled={fieldIdx === editingForm.fields.length - 1} onClick={() => {
                           const newF = [...editingForm.fields];
                           const temp = newF[fieldIdx+1];
                           newF[fieldIdx+1] = newF[fieldIdx];
                           newF[fieldIdx] = temp;
                           updateCurrentForm({...editingForm, fields:newF});
                         }} style={{background:"none",border:"none",color:fieldIdx===editingForm.fields.length-1?"#ccc":"var(--dt)",cursor:fieldIdx===editingForm.fields.length-1?"default":"pointer",fontSize:"1.2rem",marginLeft:4,lineHeight:1}}>↓</button>

                         <button onClick={() => {
                           const newF = [...editingForm.fields]; newF.splice(fieldIdx, 1); updateCurrentForm({...editingForm, fields:newF});
                 ...
                 <div style={{background:"#F9F9F9",padding:12,borderRadius:8,border:"1px solid var(--bd)",marginBottom:12}}>
                    ...
                    <div style={{display:"flex",gap:8,alignItems:"center",marginBottom:10}}>
                      <select id="sel_form_lib" style={{flex:1,padding:"8px",border:"1px solid var(--bd)",borderRadius:6,fontSize:".85rem"}}>
                        <option value="">-- Choose from Library --</option>
                        {fieldLib.map(fl => <option key={fl.id} value={fl.id}>{fl.label} ({fl.type})</option>)}
                      </select>
                      <button onClick={() => {
                        const sel = document.getElementById("sel_form_lib");
                        if(!sel || !sel.value) return;
                        const target = fieldLib.find(fl => fl.id === sel.value);
                        if(target) {
                          const updated = {...editingForm, fields: [...(editingForm.fields || []), {libId: target.id, label:target.label, type:target.type, options:target.options, required:false}]};
                          updateCurrentForm(updated);
                        }
                        sel.value = "";
                    ...
                    {!isAddingLib ? (
                      ...
                      <div style={{display:"flex",gap:6,alignItems:"center",marginTop:6,background:"white",padding:10,borderRadius:6,border:"1px dashed var(--tl)"}}>
                        ...
                        <select value={newLibType} onChange={e => setNewLibType(e.target.value)} style={{padding:"6px",fontSize:".8rem",border:"1px solid var(--bd)",borderRadius:4}}>
                          ...
                          <option value="percentage">Percentage (%) with Decimal</option>
                        </select>
                        {newLibType === 'dropdown' && (
                          <input value={newLibOptions} onChange={e => setNewLibOptions(e.target.value)} placeholder="Options (comma separated)" style={{flex:1,padding:"6px",fontSize:".8rem",border:"1px solid var(--bd)",borderRadius:4}}/>
                        )}
               ...
               {(previewForm?.fields || []).map((field, idx) => (
                 <div key={idx}>
                   ...
                   {field.type === 'address' ? (
                     ...
                     <input disabled type={field.type==='number'?'number':field.type==='email'?'email':field.type==='date'?'date':'text'} placeholder={field.label} style={{width:"100%",padding:"10px",borderRadius:8,border:"1px solid var(--bd)",fontSize:".9rem",background:"#F9F9F9"}}/>
                   )}
                 </div>
               ))}
             </div>
              ...
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>
                ...
                <div style={{gridColumn:"1/-1", marginTop:8, padding:12, background:"#FFF5F5", borderRadius:8, border:"2px solid #F5B8B8"}}>
                  <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom: ev.regClosed ? 10 : 0}}>
                    <label style={{fontSize:".8rem",fontWeight:700,display:"flex",alignItems:"center",gap:8,cursor:"pointer",color:"#C0392B"}}>
                      <input type="checkbox" checked={ev.regClosed || false} onChange={e=>updateItem(i,"regClosed",e.target.checked)} />
                      <span>{"🚫 Stop New Registrations (Show closed message to users)"}</span>
                    </label>
                    {ev.regClosed && (
                      <span style={{fontSize:".65rem",background:"#C0392B",color:"white",padding:"2px 8px",borderRadius:10,fontWeight:700}}>CLOSED</span>
                    )}
                  </div>
                  {ev.regClosed && (
                    <div style={{marginTop:8}}>
                      <label style={{display:"block",fontSize:".7rem",color:"var(--mu)",fontWeight:600,marginBottom:4}}>{"Custom Closed Message (optional)"}</label>
                      <input type="text" placeholder="New registration entries have been stopped. Please contact the organizer for more information." value={ev.regClosedMsg || ""} onChange={e=>updateItem(i,"regClosedMsg",e.target.value)} style={{width:"100%",padding:"6px",borderRadius:6,border:"1px solid #F5B8B8",fontSize:".85rem",fontFamily:"inherit"}}/>
                    </div>
                  )}
                </div>
...
function PaymentSettings({ mob, C, setC, auth }) {
  ...
  return (
    <EditorContext.Provider value={{ draft, gv, upd, moveItem, delItem, addItem, mob, exp, setExp }}>
      ...
      <Sec id="seo" icon="🔍" label="SEO & Search Engine Optimization">
        <F label="Browser Page Title (<title>)" path="seoTitle" help="Shown in browser tab and search engine results"/>
          ...
          <div>
            ...
            <div style={{display:"flex", gap:8}}>
              <select 
                value={countryCode} 
                onChange={e=>setCountryCode(e.target.value)} 
                style={{padding:"10px 8px", borderRadius:12, border:"1.5px solid var(--sf)", fontSize:".95rem", fontWeight:700, background:"#FFFFFF", color:"#111827", outline:"none", cursor:"pointer", width: 85, boxSizing: "border-box"}}
              >
                <option value="+91">+91 (IN)</option>
                <option value="+1">+1 (US)</option>
                <option value="+44">+44 (UK)</option>
              ...
              <input 
                ...
                required 
                style={{flex:1, padding:"12px 14px", borderRadius:12, border:"1.5px solid var(--sf)", fontSize:"1.05rem", fontWeight:700, outline:"none", background:"#FFFFFF", color:"#111827", transition:"all .2s", boxSizing: "border-box"}} 
...
function UserDashboard({ C, globalProfile, globalAuthToken, onClose }) {
  ...
  const handleSaveResubmission = async (updatedData) => {
    try {
      ...
      Object.keys(updatedData).forEach(k => {
        if (["id", "_submittedAt", "timestamp", "Status", "status", "Remarks", "remarks", "AdminRemarks", "logHistory", "Updated By"].includes(k)) return;
        const oldVal = String(editingReg[k] !== undefined && editingReg[k] !== null ? editingReg[k] : "").trim();
        const newVal = String(updatedData[k] !== undefined && updatedData[k] !== null ? updatedData[k] : "").trim();
        if (oldVal !== newVal) {
      ...
      const changeSummary = changedFields.length > 0
        ? `User updated: ${changedFields.join(", ")}`
        : "User resubmitted details / documents for verification review";

      const existingLogs = Array.isArray(editingReg.logHistory) ? editingReg.logHistory : [];
      const resubmitLog = {
        timestamp: new Date().toISOString(),
        actor: "User",
        action: "Resubmitted Entry",
        status: "Pending",
        remarks: changeSummary
      };
      const newLogs = [...existingLogs, resubmitLog];

      const cleanData = { ...updatedData, Status: "Pending", status: "Pending", logHistory: newLogs };
      delete cleanData.id;
  ...
  return (
    <div style={{position:"fixed",inset:0,background:"rgba(13,75,94,.8)",display:"flex",alignItems:"center",justifyContent:"center",padding:isFullScreen?0:(mob?"32px 16px":32),zIndex:9999,transition:"all 0.3s"}}
      ...
      <div style={{background:"#F8F9FA",borderRadius:isFullScreen?0:(mob?16:24),width:isFullScreen?"100%":"95%",maxWidth:isFullScreen?"100%":1000,height:isFullScreen?"100%":(mob?"75vh":"85vh"),display:"flex",flexDirection:"column",boxShadow:"0 32px 80px rgba(0,0,0,.3)",position:"relative",overflow:"hidden",transition:"all 0.3s"}}
        ...
        <div style={{display:"flex",flexDirection:mob?"column":"row",flex:1,minHeight:0}}>
          ...
          <div style={{flex:1,padding:mob?"20px 16px":"32px",overflowY:"auto",background:"#F8F9FA"}}>
                            ...
                            <tr key={r.id || i} style={{borderBottom:"1px solid var(--ww)",background:i%2===0?"white":"#FAFAFA"}}>
                              ...
                              <td style={{padding:"14px 16px",whiteSpace:"nowrap"}}>
                                <span style={{background:sc.bg,color:sc.col,padding:"5px 12px",borderRadius:20,fontSize:".75rem",fontWeight:700,border:`1px solid ${sc.col}33`}}>
                                  {(r.Status || r.status || "Pending") === "Pending" ? "Under Verification" : (r.Status || r.status)}
                                </span>
                              </td>
                              <td style={{padding:"14px 16px",color:"var(--tm2)"}}>
                                ...
                                <button onClick={() => setViewingHistoryModal(r)} style={{marginTop:6,padding:"4px 10px",borderRadius:12,background:"#FFF4EC",border:"1px solid #FDDBB8",color:"var(--sf)",fontSize:".72rem",fontWeight:700,cursor:"pointer",display:"inline-flex",alignItems:"center",gap:4,boxShadow:"0 1px 3px rgba(0,0,0,0.05)"}}>
                                  📜 View History {r.logHistory && r.logHistory.length > 0 ? `(${r.logHistory.length})` : ""}
                                </button>
                              </td>
                              <td style={{padding:"14px 16px",textAlign:"center",whiteSpace:"nowrap"}}>
                                ...
                                ) : ((r.Status || r.status) === "Disapproved" || (r.Status || r.status) === "Rejected") ? (
                                  <span style={{fontSize:".75rem",fontWeight:700,color:"#C0392B",background:"#FEF0EF",padding:"4px 10px",borderRadius:12,border:"1px solid #F5B8B8"}}>
                                    🚫 Not Eligible
                                  </span>
                                ) : (
                                  <span style={{fontSize:".75rem",color:"var(--mu)",fontWeight:600}}>🔒 Locked</span>
                                )}
                              </td>
                              {rowKeys.map(k => {
                                const val = r[k] || "-";
            ...
            )}{activeTab !== "Registrations" && activeTab !== "Receipts" && activeTab !== "Awards" && activeTab !== "Invites" && activeTab !== "Profile" && (
              <div style={{background:"white",padding:"60px 20px",borderRadius:16,textAlign:"center",border:"1px solid var(--bd)",height:"100%",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center"}}>
      ...
      {viewingHistoryModal && (
        <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.65)",zIndex:99999,display:"flex",alignItems:"center",justifyContent:"center",padding:16}} onClick={()=>setViewingHistoryModal(null)}>
          <div style={{background:"white",borderRadius:16,maxWidth:520,width:"100%",maxHeight:"80vh",display:"flex",flexDirection:"column",overflow:"hidden",boxShadow:"0 20px 40px rgba(0,0,0,0.3)"}} onClick={e=>e.stopPropagation()}>
            <div style={{padding:"16px 20px",background:"linear-gradient(135deg, #1e3a8a, #312e81)",color:"white",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
              <div>
                <h3 style={{fontSize:"1.1rem",fontWeight:700,margin:0}}>📜 Activity & Verification Log</h3>
                <div style={{fontSize:".75rem",opacity:0.8,marginTop:2}}>ID: {viewingHistoryModal.id || "N/A"}</div>
              </div>
              <button onClick={()=>setViewingHistoryModal(null)} style={{background:"rgba(255,255,255,0.2)",border:"none",color:"white",borderRadius:"50%",width:32,height:32,cursor:"pointer",fontWeight:700,fontSize:"1.1rem"}}>✕</button>
            </div>

            <div style={{padding:20,overflowY:"auto",flex:1,display:"flex",flexDirection:"column",gap:12}}>
              {(() => {
                const logs = (viewingHistoryModal.logHistory && viewingHistoryModal.logHistory.length > 0)
                  ? viewingHistoryModal.logHistory
                  : [
                      {
                        actor: "User",
                        action: "1st Entry Submitted",
                        timestamp: viewingHistoryModal.timestamp || viewingHistoryModal._submittedAt,
                        remarks: "Initial registration entry submitted"
                      },
                      ...(viewingHistoryModal.Remarks || viewingHistoryModal.remarks || viewingHistoryModal.AdminRemarks ? [{
                        actor: viewingHistoryModal['Updated By'] || "Admin",
                        action: `Status: ${viewingHistoryModal.Status || viewingHistoryModal.status || 'Under Verification'}`,
                        timestamp: viewingHistoryModal.updatedAt || viewingHistoryModal.timestamp,
                        remarks: viewingHistoryModal.Remarks || viewingHistoryModal.remarks || viewingHistoryModal.AdminRemarks
                      }] : [])
                    ];
                ...
                return logs.map((lg, lgIdx) => (
                  <div key={lgIdx} style={{background:"#FAFAFA",border:"1.5px solid var(--bd)",borderRadius:10,padding:14,position:"relative"}}>
                    <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:6}}>
                      <span style={{fontSize:".78rem",fontWeight:800,color:lg.actor === 'User' ? "var(--sf)" : "#1E3A8A"}}>
                        {lg.actor === 'User' ? "👤 You (Registrant)" : `🛡️ Admin (${lg.actor || 'Admin'})`}
                      </span>
...
function AdminRegistrations({ mob, C, setC, auth }) {
  ...
  const saveVerification = async (r, newStatus, newRemarks) => {
    ...
    const adminLog = {
      ...
      status: newStatus,
      remarks: newRemarks || "Admin updated verification status"
    };
  ...
  const handleStatusChange = async (r, newStatus) => {
    ...
    const adminLog = {
      ...
      status: newStatus,
      remarks: newRemarks || "Admin changed status"
    };
    const newLogs = [...existingLogs, adminLog];
      ...
      {historyModalReg && (
        <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.65)",zIndex:100000,display:"flex",alignItems:"center",justifyContent:"center",padding:16}} onClick={()=>setHistoryModalReg(null)}>
          <div style={{background:"white",borderRadius:16,maxWidth:540,width:"100%",maxHeight:"85vh",display:"flex",flexDirection:"column",overflow:"hidden",boxShadow:"0 20px 40px rgba(0,0,0,0.3)"}} onClick={e=>e.stopPropagation()}>
            <div style={{padding:"16px 20px",background:"linear-gradient(135deg, #1e3a8a, #312e81)",color:"white",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
              <div>
                <h3 style={{fontSize:"1.1rem",fontWeight:700,margin:0}}>📜 Registration Activity & Audit Log</h3>
                <div style={{fontSize:".75rem",opacity:0.8,marginTop:2}}>
                  ID: {historyModalReg.id || historyModalReg['Transaction ID'] || "N/A"} • Submitter: {historyModalReg.submitterMob || historyModalReg['Mobile Number'] || historyModalReg.phone || "N/A"}
                </div>
              </div>
              ...
              {(() => {
                const logs = (historyModalReg.logHistory && historyModalReg.logHistory.length > 0)
                  ...
                  : [
                      ...
                      ...(historyModalReg.Remarks || historyModalReg.remarks || historyModalReg.AdminRemarks ? [{
                        actor: historyModalReg['Updated By'] || "Admin",
                        action: `Status: ${historyModalReg.Status || historyModalReg.status || 'Pending'}`,
                        timestamp: historyModalReg.updatedAt || historyModalReg.timestamp,
                        remarks: historyModalReg.Remarks || historyModalReg.remarks || historyModalReg.AdminRemarks
                      }] : [])
                    ];
                ...
                return logs.map((lg, lgIdx) => (
                  <div key={lgIdx} style={{background:"#FAFAFA",border:"1.5px solid var(--bd)",borderRadius:10,padding:14,position:"relative"}}>
                    <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:6}}>
                      <span style={{fontSize:".78rem",fontWeight:800,color:lg.actor === 'User' ? "var(--sf)" : "#1E3A8A"}}>
                        {lg.actor === 'User' ? "👤 User (Registrant)" : `🛡️ Admin (${lg.actor || 'Admin'})`}
                      </span>
                      <span style={{fontSize:".7rem",color:"var(--mu)",fontWeight:600}}>
                        {lg.timestamp ? new Date(lg.timestamp).toLocaleString() : ""}
                      </span>
...
export default function App() {
  ...
  useEffect(() => {
    if (C) {
      const pageTitle = C.seoTitle || (C.trust?.name ? `${C.trust.name} | Community Portal` : "Community Portal");
      document.title = pageTitle;

      const setMetaTag = (selector, nameAttr, contentVal) => {
        if (!contentVal) return;
        let el = document.querySelector(selector);
        if (!el) {
          el = document.createElement('meta');
          if (selector.includes('property=')) {
            el.setAttribute('property', selector.split('property="')[1].replace('"]', ''));
          } else {
      ...
      setMetaTag('meta[name="description"]', 'description', C.seoDescription);
      setMetaTag('meta[name="keywords"]', 'keywords', C.seoKeywords);
      setMetaTag('meta[name="author"]', 'author', C.seoAuthor);
      setMetaTag('meta[property="og:title"]', 'og:title', C.seoOgTitle || pageTitle);
      setMetaTag('meta[property="og:description"]', 'og:description', C.seoOgDesc || C.seoDescription);
      setMetaTag('meta[property="twitter:title"]', 'twitter:title', C.seoOgTitle || pageTitle);
      setMetaTag('meta[property="twitter:description"]', 'twitter:description', C.seoOgDesc || C.seoDescription);
  ...
  }, [C?.seoTitle, C?.seoDescription, C?.seoKeywords, C?.seoAuthor, C?.seoOgTitle, C?.seoOgDesc, C?.trust?.name, C?.trust?.logo?.url]);
        ...
        <div style={{minHeight:"100vh",background:"#F4F7FB",padding:"20px 12px",display:"flex",justifyContent:"center",alignItems:"flex-start"}}>
          <div style={{width:"100%",maxWidth:550}}>
            <div style={{textAlign:"center",marginBottom:20}}>
              <LogoMark logo={{...C.trust?.logo, size: 50}} mob={false} />
              <h2 style={{fontFamily:"'Playfair Display',serif",color:"var(--dt)",fontSize:"1.4rem",marginTop:8,marginBottom:4}}>{C.trust?.name}</h2>
              <p style={{fontSize:".8rem",color:"var(--mu)",margin:0}}>Official Event Registration Portal</p>
            </div>
            <Events C={C} lang={lang} globalAuthToken={auth?.idToken} globalProfile={null} onPublicLogin={()=>{}} forceStandalone={true} />
          </div>
        </div>
      ) : page === "public" ? (
